...
def initParams():
    ...
    global crash, done, t_ep, t_per_episode, t_sim_start, t_step
        ...
        while not rospy.is_shutdown():
            ...
            if step_time > MIN_TIME_BETWEEN_ACTIONS:
                ...
                if episode > MAX_EPISODES:
                    ...
                    # End of en Episode
                    if crash or done or ep_steps >= MAX_STEPS_PER_EPISODE:
                        robotStop(velPub)
                        if crash:
                            # get crash position
                            ...
                            # get Goal position
                            odomMsg = rospy.wait_for_message('/odom', Odometry)
                            ( x_goal1 , y_goal1 ) = getPosition(odomMsg)
                            theta_goal1 = degrees(getRotation(odomMsg))
                        t_ep = rospy.Time.now()
                        ...
                        if crash:
                            ...
                            text = text + '\r\nEpisode %d ==> CRASH {%.2f,%.2f,%.2f}    ' % (episode, x_goal1, y_goal1, theta_goal1) + dt_string
                        ...
                        if not robot_in_pos:
                            ...
                            if RANDOM_INIT_POS:
                                ...
                                ( x_goal , y_goal , theta_goal ) = (1,2,40)
                            else:
                        ...
                        # Rest of the algorithm
                            ...
                            ( state_ind, x1, x2 ,x3 ,x4 ) = scanDiscretization(state_space, lidar)
                            crash = checkCrash(lidar)
                            if (checkGoalNear(x,y,x_goal,y_goal)<0.3):
                                done = True
                            else:    
                                done = False
                            ( reward, terminal_state ) = getReward(action, prev_action, lidar, prev_lidar, crash, done)
